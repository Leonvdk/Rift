  ...
  return (
    <header className="sticky top-0 z-50 bg-cream/95 shadow-[0_1px_8px_rgba(0,0,0,0.06)] backdrop-blur-sm">
      <div className="rift-container flex items-center justify-between py-4 md:py-5">
        ...
        <button
          type="button"
          className="relative z-50 pt-1 md:hidden"
        ...
        {/* Mobile menu overlay */}
        <div
          className={`fixed inset-0 z-40 bg-cream transition-all duration-500 ease-out md:hidden ${
            open
              ? "opacity-100 visible"
              : "opacity-0 invisible"
          }`}
        >
          <nav className="flex h-full flex-col justify-center gap-10 px-8">
            {navItems.map((item, i) => {
              const isActive = pathname === item.href
              return (
                <Link
